...
// Helper to format the scheduled time exactly like the image (e.g., "Tue 9:15:12 AM")
...
export function ScheduledEmailsTab() {
  ...
  // Fetch data - assuming endpoint supports limit/offset
  ...
  if (error) {
    return (
      <div className="m-6 p-4 bg-red-50 border border-red-100 rounded-lg flex gap-3 items-center text-red-600">
        ...
        <span className="text-sm font-medium">Failed to load scheduled emails.</span>
      </div>
    );
  }
  ...
  if (emails.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="text-gray-900 font-medium">No scheduled emails</p>
        <p className="text-gray-500 text-sm mt-1">Emails scheduled for later will appear here.</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col">
        {emails.map((email) => (
          ...
          >
            ...
            {/* Scheduled Badge (Orange with Clock) */}
            <div className="flex-shrink-0 mr-4">
              <span className="inline-flex items-center justify-center gap-1.5 px-3 py-1 rounded-full bg-[#FFF3E0] text-[#E65100] border border-[#FFE0B2] text-xs font-medium min-w-[140px]">
                <Clock className="w-3.5 h-3.5" />
                {formatScheduledTime(email.scheduledAt || email.startTime)}
              </span>
            </div>
